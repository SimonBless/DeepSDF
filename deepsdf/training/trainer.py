...
class LatentCodeManager:
    ...
    def get_codes(self, indices: torch.Tensor) -> torch.Tensor:
        ...
        return self.latent_codes(indices)
...
class Trainer:
    ...
    def __init__(
        ...
        num_shapes = len(train_loader.dataset)
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            indices = torch.arange(
                ...
                min((batch_idx + 1) * self.config.training.batch_size, len(self.train_loader.dataset))
            ).to(self.device)
    ...
    def validate(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(self.val_loader):
                ...
                indices = torch.arange(
                    batch_idx * self.config.training.batch_size,
                    min((batch_idx + 1) * self.config.training.batch_size, len(self.val_loader.dataset))
                ).to(self.device)
